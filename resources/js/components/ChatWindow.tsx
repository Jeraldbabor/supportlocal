...
export default function ChatWindow({ conversationId, currentUserId }: ChatWindowProps) {
    ...
    const startPolling = useCallback(() => {
        ...
        pollingIntervalRef.current = setInterval(() => {
            loadMessages(true);
        }, 1000); // Poll every second for faster fallback delivery

        // Trigger immediate sync so the first fallback receive is not delayed
        loadMessages(true);
    }, [loadMessages]);
    ...
    const sendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Remove failed optimistic message and restore draft text for retry
    ...
    if (!conversation) {
        return (
            <div className="flex h-full items-center justify-center bg-gray-50" data-chat-container>
