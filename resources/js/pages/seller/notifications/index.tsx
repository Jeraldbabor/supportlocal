...
function NotificationsPageContent({ notifications }: NotificationsProps) {
    ...
    return (
        <div className="mx-auto max-w-4xl px-3 sm:px-4 py-4 sm:py-8 lg:px-8">
            <div className="rounded-lg bg-white shadow dark:bg-gray-800">
                {/* Header */}
                <div className="border-b border-gray-200 px-3 sm:px-4 lg:px-6 py-3 sm:py-4 dark:border-gray-700">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                        <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                            <Bell className="h-5 w-5 sm:h-6 sm:w-6 text-blue-600 flex-shrink-0" />
                            <h1 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white truncate">Notifications</h1>
                            {unreadCount > 0 && (
                                <span className="rounded-full bg-red-100 px-2 sm:px-2.5 py-0.5 text-[10px] sm:text-xs font-medium text-red-800 flex-shrink-0 whitespace-nowrap">{unreadCount} unread</span>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                            {notifications.data.length > 0 && (
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 sm:gap-2 rounded-lg border border-red-300 bg-white px-2 sm:px-3 py-1.5 text-xs sm:text-sm font-medium text-red-700 transition-colors hover:bg-red-50 whitespace-nowrap"
                                    disabled={isClearing}
                            ...
                            {unreadCount > 0 && (
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 sm:gap-2 rounded-lg border border-blue-300 bg-white px-2 sm:px-3 py-1.5 text-xs sm:text-sm font-medium text-blue-700 transition-colors hover:bg-blue-50 whitespace-nowrap"
                                    title="Mark All Read"
                                >
                ...
                <div className="divide-y divide-gray-200 dark:divide-gray-700">
                    {notifications.data.length === 0 ? (
                        <div className="px-3 sm:px-4 lg:px-6 py-8 sm:py-12 text-center">
                            ...
                            <h3 className="mt-2 text-xs sm:text-sm font-medium text-gray-900 dark:text-white">No notifications</h3>
                        ...
                        notifications.data.map((notification) => (
                            <div
                                key={notification.id}
                                className={`px-3 sm:px-4 lg:px-6 py-3 sm:py-4 transition-all duration-200 ${
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {notification.data.admin_notes && (
                                                    <div className="mt-2 rounded-md border border-blue-200 bg-blue-50 p-2 sm:p-3 dark:border-blue-800 dark:bg-blue-900/30">
                                                        <p className="mb-1 text-[10px] sm:text-xs font-semibold text-blue-900 dark:text-blue-200">
                                                            Note from Admin:
                                                        </p>
                                                        <p className="text-xs sm:text-sm text-blue-800 dark:text-blue-300">{notification.data.admin_notes}</p>
                                                    </div>
                                                )}
                                                <div className="mt-2 flex items-center text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
                                            ...
                                            <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                                                <button
                                                    ...
                                                    className="rounded-full p-1 text-red-500 transition-colors hover:bg-red-50 hover:text-red-700 disabled:opacity-50 flex-shrink-0"
                                                ...
                                                {!isRead(notification) ? (
                                                    <button
                                                        ...
                                                        className="text-[10px] sm:text-xs font-medium text-blue-600 hover:text-blue-800 whitespace-nowrap flex-shrink-0"
                                                    >
                                                        Mark as read
                                                    </button>
                                                ) : (
                                                    <span className="flex items-center text-[10px] sm:text-xs text-green-600 whitespace-nowrap flex-shrink-0">
                ...
                {notifications.total > notifications.per_page && (
                    <div className="border-t border-gray-200 px-3 sm:px-4 lg:px-6 py-3 sm:py-4 dark:border-gray-700">
                            ...
                            <div className="flex items-center justify-center gap-2">
                                {notifications.current_page > 1 && (
                                    <button
                                        ...
                                        className="rounded-md border border-gray-300 px-2.5 sm:px-3 py-1 text-xs sm:text-sm hover:bg-gray-50 whitespace-nowrap"
                                    >
                                        Previous
                                    </button>
                                )}
